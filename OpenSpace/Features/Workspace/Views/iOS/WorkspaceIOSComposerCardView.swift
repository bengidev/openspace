//
//  WorkspaceIOSComposerCardView.swift
//  OpenSpace
//
...
// MARK: - WorkspaceIOSComposerCard

struct WorkspaceIOSComposerCard: View {
    // MARK: Internal
    ...
    @Binding var activeProviderPopup: WorkspaceIOSProviderPopup?
    ...
    private var composerControls: some View {
        if context.composerUsesCompactFallback {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 10) {
                    WorkspaceIOSSurfaceChip(title: "Attach", systemImage: "paperclip", colorScheme: colorScheme, context: context)
                    providerMenu
                }
...
#Preview("iPhone Workspace Composer Card") {
    WorkspacePreviewSupport.preview(
        variant: .ios,
        size: CGSize(width: 390, height: 844),
        selectedDestination: .home,
        selectedPrompt: "Draft a concise release update for the design review thread."
    ) { context, bindings in
