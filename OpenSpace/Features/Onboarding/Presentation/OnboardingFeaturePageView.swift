...
struct OnboardingFeaturePageView: View {
    ...
    var body: some View {
        if page.model == .workspaceReady {
            OnboardingWorkspaceReadyVisual(page: page, palette: palette, appeared: appeared)
                            ...
                            VStack(spacing: 0) {
                                ...
                                Button(action: onAddQueuedPrompt) {
                                    HStack(spacing: 8) {
                                        ...
                                        Text(queuedPromptCount >= OnboardingPromptQueueItem.samples.count ? "RESET QUEUE" : "ADD FOLLOW-UP")
                                        Spacer()
                                        Text("⌘↩")
                                            .font(.system(size: 10, weight: .semibold, design: .monospaced))
                                            .tracking(-0.24)
    ...
    private var visualBody: some View {
        ...
        case .encryptedPairing:
            OnboardingEncryptedPairingVisual(
                palette: palette,
        ...
        case .reasoningControl:
            OnboardingReasoningControlVisual(
                palette: palette,
        ...
        case .workspaceReady:
            OnboardingWorkspaceReadyVisual(
                page: page,
                palette: palette,
                appeared: appeared
            )
