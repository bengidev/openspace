...
struct OnboardingMacLayout {
    ...
    var screenContentMinHeight: CGFloat {
        max(context.containerSize.height - (screenVerticalPadding * 2), 0)
    }

    var panelTopPadding: CGFloat {
        isCompactHeight ? 16 : (isExpandedHeight ? 34 : 22)
    }

    var panelBottomPadding: CGFloat {
        isCompactHeight ? 16 : (isExpandedHeight ? 28 : 22)
    }

    var panelHorizontalInset: CGFloat {
        isCompactWidth ? 18 : (isExpandedHeight ? 32 : 24)
    }

    var sectionSpacing: CGFloat {
        prefersStackedHero ? 12 : 16
    }

    var heroColumnSpacing: CGFloat {
        prefersStackedHero ? 18 : 28
    }
    ...
    var secondaryColumnWidth: CGFloat {
        min(max(context.containerSize.width * 0.24, 300), 360)
    }

    var titleTopPadding: CGFloat {
        prefersStackedHero ? 16 : 22
    }

    var actionTopPadding: CGFloat {
        isCompactHeight ? 34 : (isExpandedHeight ? 72 : 56)
    }

    var actionBottomPadding: CGFloat {
        isCompactHeight ? 4 : 8
    }
    ...
    var heroSubtitleFont: Font {
        context.containerSize.width < 920 ? .body : .title3
    }

    var heroTextMaxWidth: CGFloat? {
        prefersStackedHero ? nil : 560
    }

    var heroSupportingTextMaxWidth: CGFloat? {
        prefersStackedHero ? nil : 540
    }
    ...
    var heroPrimaryButtonMinWidth: CGFloat? {
        prefersStackedHero ? nil : 240
    }
