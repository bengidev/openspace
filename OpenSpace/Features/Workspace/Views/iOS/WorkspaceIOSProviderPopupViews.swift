//
//  WorkspaceIOSProviderPopupViews.swift
//  OpenSpace
//
//  iPhone-focused provider picker and connection popup views.
//

import SwiftUI

// MARK: - WorkspaceIOSProviderPickerPopup

struct WorkspaceIOSProviderPickerPopup: View {
    // MARK: Internal
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            WorkspaceIOSProviderPickerHeader(dismiss: dismiss)
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(WorkspaceIOSProviderPickerPalette.background)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .stroke(WorkspaceIOSProviderPickerPalette.stroke, lineWidth: 1)
        )
        .compositingGroup()
        ...
        ScrollView(.vertical, showsIndicators: true) {
            LazyVStack(alignment: .leading, spacing: rowSpacing) {
                if filteredProviders.isEmpty {
                    WorkspaceIOSProviderEmptyState(searchText: searchText)
...
// MARK: - WorkspaceIOSProviderPickerHeader

struct WorkspaceIOSProviderPickerHeader: View {
    let dismiss: () -> Void
    ...
    var body: some View {
        HStack(alignment: .center) {
            Text("Connect provider")
                ...
                .foregroundStyle(WorkspaceIOSProviderPickerPalette.primaryText)
            ...
            Button(action: dismiss) {
                Image(systemName: "xmark")
                    ...
                    .foregroundStyle(WorkspaceIOSProviderPickerPalette.icon)
...
// MARK: - WorkspaceIOSProviderSearchField

struct WorkspaceIOSProviderSearchField: View {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(WorkspaceIOSProviderPickerPalette.fieldBackground)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(WorkspaceIOSProviderPickerPalette.stroke.opacity(0.74), lineWidth: 1)
        )
...
// MARK: - WorkspaceIOSProviderPickerRow

struct WorkspaceIOSProviderPickerRow: View {
    ...
    var body: some View {
        Button(action: select) {
            HStack(alignment: .center, spacing: rowSpacing) {
                WorkspaceIOSProviderIcon(provider: provider)
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(providerSummary)
                        ...
                        .foregroundStyle(WorkspaceIOSProviderPickerPalette.secondaryText)
                        .lineLimit(1)
                }
                ...
                if isSelected {
                    Image(systemName: "checkmark")
                        ...
                        .foregroundStyle(WorkspaceIOSProviderPickerPalette.badgeText)
                        .frame(width: 28, height: 28)
                        .background(
                            Circle()
                                .fill(WorkspaceIOSProviderPickerPalette.selectedIndicatorBackground)
                        )
                        .accessibilityHidden(true)
                }
            }
    ...
    private var rowBackground: some View {
        RoundedRectangle(cornerRadius: 12, style: .continuous)
            .fill(
                isSelected
                    ? WorkspaceIOSProviderPickerPalette.selectedBackground
                    : Color.clear
            )
    }
    ...
    private var rowStroke: some View {
        RoundedRectangle(cornerRadius: 12, style: .continuous)
            .stroke(
                isSelected
                    ? WorkspaceIOSProviderPickerPalette.selectedStroke
                    : Color.clear,
                lineWidth: 1
            )
    }
...
// MARK: - WorkspaceIOSProviderIcon

struct WorkspaceIOSProviderIcon: View {
    let provider: AIProvider

    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .fill(WorkspaceIOSProviderPickerPalette.iconBackground)
                .overlay(
                    RoundedRectangle(cornerRadius: 9, style: .continuous)
                        .stroke(WorkspaceIOSProviderPickerPalette.stroke, lineWidth: 1)
                )
...
// MARK: - WorkspaceIOSProviderEmptyState

struct WorkspaceIOSProviderEmptyState: View {
...
// MARK: - WorkspaceIOSProviderConnectionPopup

struct WorkspaceIOSProviderConnectionPopup: View {
    // MARK: Lifecycle
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            WorkspaceIOSProviderConnectionNavigation(dismiss: dismiss, back: back)
                .padding(.top, popupTopPadding)
                .padding(.horizontal, popupHorizontalPadding)
                .padding(.bottom, 10)
        ...
        case .apiKey:
            VStack(alignment: .leading, spacing: 9) {
                ...
                SecureField("API key", text: $apiKey)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .stroke(WorkspaceIOSProviderPickerPalette.stroke.opacity(0.82), lineWidth: 1)
                    )
...
// MARK: - WorkspaceIOSProviderConnectionFooter

struct WorkspaceIOSProviderConnectionFooter: View {
    let title: String
    let isDisabled: Bool
    let action: () -> Void
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(.system(size: 17, weight: .semibold))
                .foregroundStyle(WorkspaceIOSProviderPickerPalette.primaryButtonText)
                .frame(maxWidth: .infinity)
                .frame(height: buttonHeight)
                .background(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(WorkspaceIOSProviderPickerPalette.primaryButtonBackground)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .stroke(WorkspaceIOSProviderPickerPalette.primaryButtonStroke, lineWidth: 1)
                )
        }
        .buttonStyle(.plain)
        .disabled(isDisabled)
        ...
        .padding(.bottom, 16)
        .background(
            WorkspaceIOSProviderPickerPalette.background
                .overlay(alignment: .top) {
                    Rectangle()
                        .fill(WorkspaceIOSProviderPickerPalette.stroke.opacity(0.62))
                        .frame(height: 1)
                }
        )
    }
...
// MARK: - WorkspaceIOSProviderConnectionNavigation

struct WorkspaceIOSProviderConnectionNavigation: View {
    let dismiss: () -> Void
    let back: () -> Void
...
// MARK: - WorkspaceIOSProviderConnectionMethodPicker

struct WorkspaceIOSProviderConnectionMethodPicker: View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(spacing: 8) {
                ForEach(methods) { method in
                    WorkspaceIOSProviderConnectionMethodRow(
                        method: method,
...
// MARK: - WorkspaceIOSProviderConnectionMethodRow

struct WorkspaceIOSProviderConnectionMethodRow: View {
    ...
    var body: some View {
        Button(action: select) {
            HStack(alignment: .top, spacing: 13) {
                ZStack {
                    Circle()
                        .stroke(
                            isSelected
                                ? WorkspaceIOSProviderPickerPalette.primaryButtonBackground
                                : WorkspaceIOSProviderPickerPalette.stroke,
                            lineWidth: 1.6
                        )
            ...
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(isSelected ? WorkspaceIOSProviderPickerPalette.selectedBackground : WorkspaceIOSProviderPickerPalette.fieldBackground)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        isSelected
                            ? WorkspaceIOSProviderPickerPalette.selectedStroke
                            : WorkspaceIOSProviderPickerPalette.stroke.opacity(0.74),
                        lineWidth: 1
                    )
            )
            .contentShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
...
// MARK: - WorkspaceIOSCenteredProviderPopupOverlay

struct WorkspaceIOSCenteredProviderPopupOverlay<Content: View>: View {
    let dismiss: () -> Void
...
// MARK: - WorkspaceIOSProviderPickerPalette

enum WorkspaceIOSProviderPickerPalette {
