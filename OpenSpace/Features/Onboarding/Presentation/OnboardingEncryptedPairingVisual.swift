...
struct OnboardingEncryptedPairingVisual: View {
    ...
    var body: some View {
        ZStack {
            HStack(spacing: 0) {
                OnboardingDeviceNode(title: "LOCAL", subtitle: "Local key", systemImage: "iphone", palette: palette, active: isConfirmed)
                    .offset(x: appeared ? 0 : -24)
                Spacer(minLength: 6)
                OnboardingDeviceNode(title: "OPENSPACE", subtitle: "AI chat lane", systemImage: "macbook", palette: palette, active: true)
                    .offset(x: appeared ? 0 : 24)
            }
